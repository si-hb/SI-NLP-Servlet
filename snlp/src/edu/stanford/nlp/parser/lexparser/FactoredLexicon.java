...
public class FactoredLexicon extends BaseLexicon {
  ...
  public Iterator<IntTaggedWord> ruleIteratorByWord(int word, int loc, String featureSpec) {
    ...
    if(wordTaggings.size() == 0){
      ...
      if(wordTaggings.size() == 0) {
        String wordForm = word < wordIndex.size() ? wordIndex.get(word) : UNKNOWN_WORD;
        System.err.printf("%s: Unseen word/feature combination: %s %s%n",
    ...
    if(wordTaggings.size() == 0) {
      throw new RuntimeException(String.format("%s: No lexical insertion rules for (%s): %s%n", 
          this.getClass().getName(),
          wordIndex.get(word),
          feats.toString()));
    }
  ...
  /**
   * Has options for both strict and soft matching via the flexiTag option.
   * @param word
   * @param wordFeats
   * @return
   */
  private List<IntTaggedWord> rulesForWord(int word, MorphoFeatures wordFeats) {
    ...
    for (IntTaggedWord iTW : rulesWithWord[word]) {
      ...
      boolean isStrictMatch = tagFeats.numActiveFeatures() == wordFeats.numActiveFeatures();
      isStrictMatch = isStrictMatch && (wordFeats.numActiveFeatures() == nMatches);
      ...
      } else if(isStrictMatch) {//Strict feature match
        if(DEBUG) System.err.println(tagStr);
        tagRules.add(iTW);
