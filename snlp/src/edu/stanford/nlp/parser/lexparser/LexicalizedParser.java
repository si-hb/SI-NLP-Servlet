...
public class LexicalizedParser implements Function<Object,Tree>, Serializable {
   ...
   * Yuri added
   */
  private static void printOptions(boolean train, Options op, PrintWriter pw) {
    op.display();
    if (train) {
      op.trainOptions.display(pw);
    } else {
      op.testOptions.display(pw);
    }
    op.tlpParams.display(pw);
  }
   ...
   * Yuri added
   */
  private static void printArgs(String[] args, PrintWriter pw) {
	  pw.print("LexicalizedParser invoked with arguments:");
    for (String arg : args) {
    	pw.print(' ' + arg);
    }
    pw.println();
  ...
  /*
  * Yuri added - Another main for web access (only use pure sentence parameter)
  *
  * @param args command line as web parameter
  * @param sentence_text sentece to parse as web parameter
  */
  public static void web_main(String[] args, String sentence_text, PrintWriter writer, String root_path) {
    boolean train = false;
    boolean saveToSerializedFile = false;
    boolean saveToTextFile = false;
    String serializedInputFileOrUrl = null;
    ...
    int argIndex = 0;
    if (args.length < 1) {
      writer.println("Basic usage (see Javadoc for more): java edu.stanford.nlp.parser.lexparser.LexicalizedParser parserFileOrUrl filename*");
      return;
    }
    ...
    while (argIndex < args.length && args[argIndex].charAt(0) == '-') {
      if (args[argIndex].equalsIgnoreCase("-train") ||
        ...
        try {
          ...
          writer.println("Class not found: " + args[argIndex + 1]);
          throw new RuntimeException(e);
        } catch (InstantiationException e) {
          writer.println("Couldn't instantiate: " + args[argIndex + 1] + ": " + e.toString());
          throw new RuntimeException(e);
        } catch (IllegalAccessException e) {
          writer.println("Illegal access" + e);
          throw new RuntimeException(e);
        }
        argIndex += 2;
      } else if (args[argIndex].equalsIgnoreCase("-encoding")) {
        ...
        try {
          ...
          writer.println("Couldn't instantiate escaper " + args[argIndex + 1] + ": " + e);
        }
        argIndex += 2;
      } else if (args[argIndex].equalsIgnoreCase("-tokenizerOptions")) {
        tokenizerOptions = args[argIndex + 1];
        ...
        saveToSerializedFile = true;
        if (numSubArgs(args, argIndex) < 1) {
          writer.println("Missing path: -saveToSerialized filename");
        } else {
          serializedOutputFileOrUrl = args[argIndex + 1];
        }
        argIndex += 2;
    ...
    if (tokenizerFactoryClass != null || tokenizerOptions != null) {
      try {
    	...
    	writer.println("Couldn't instantiate TokenizerFactory " + tokenizerFactoryClass + " with options " + tokenizerOptions);
        throw new RuntimeException(e);
      } catch (NoSuchMethodException e) {
    	writer.println("Couldn't instantiate TokenizerFactory " + tokenizerFactoryClass + " with options " + tokenizerOptions);
        throw new RuntimeException(e);
    ...
    if (tuneFilter != null) {
      if (tunePath == null) {
        if (treebankPath == null) {
          ...
          writer.println("No tune treebank path specified.  Using train path: \"" + treebankPath + '\"');
          tunePath = treebankPath;
        }
      }
      tuneTreebank = op.tlpParams.testMemoryTreebank();
    ...
    if (!train && op.testOptions.verbose) {
      writer.println("Currently " + new Date());
      printArgs(args, writer);
    }
    LexicalizedParser lp; // always initialized in next if-then-else block
    if (train) {
      printArgs(args, writer);
      ...
      if (serializedInputFileOrUrl == null) {
    	writer.println("No grammar specified, exiting...");
        return;
      }
      String[] extraArgs = new String[optionArgs.size()];
      extraArgs = optionArgs.toArray(extraArgs);
      try {
        lp = loadModel(root_path + serializedInputFileOrUrl, op, extraArgs);
        op = lp.op;
      } catch (IllegalArgumentException e) {
    	writer.println("Error loading parser, exiting...");
        throw e;
      }
    }
    ...
    if (testFilter != null || testPath != null) {
      if (testPath == null) {
        if (treebankPath == null) {
          ...
          writer.println("No test treebank path specified.  Using train path: \"" + treebankPath + '\"');
          testPath = treebankPath;
        }
      }
      testTreebank = op.tlpParams.testMemoryTreebank();
    ...
    if (saveToTextFile) {
      ...
      if (textOutputFileOrUrl != null) {
    	...
    	writer.println("Usage: must specify a text grammar output path");
      }
    }
    if (saveToSerializedFile) {
      if (serializedOutputFileOrUrl != null) {
        lp.saveParserToSerialized(root_path + serializedOutputFileOrUrl);
      } else if (textOutputFileOrUrl == null && testTreebank == null) {
        // no saving/parsing request has been specified
    	writer.println("usage: " + "java edu.stanford.nlp.parser.lexparser.LexicalizedParser " + "-train trainFilesPath [fileRange] -saveToSerializedFile serializedParserFilename");
      }
    }
    ...
    if (op.testOptions.verbose || train) {
      ...
      // get lexicon size separately as it may have its own prints in it....
      String lexNumRules = lp.lex != null ? Integer.toString(lp.lex.numRules()): "";
      writer.println("Grammar\tStates\tTags\tWords\tUnaryR\tBinaryR\tTaggings");
      writer.println("Grammar\t" +
          ...
          (lp.bg != null ? lp.bg.numRules(): "") + '\t' +
          lexNumRules);
      writer.println("ParserPack is " + op.tlpParams.getClass().getName());
      writer.println("Lexicon is " + lp.lex.getClass().getName());
      if (op.testOptions.verbose) {
    	writer.println("Tags are: " + lp.tagIndex);
        // System.err.println("States are: " + lp.pd.stateIndex); // This is too verbose. It was already printed out by the below printOptions command if the flag -printStates is given!
      }
      printOptions(false, op);
    }
    ...
    if (testTreebank != null) {
      ...
      // no more arguments, so we just parse our own test sentence
      PrintWriter pwOut = writer;//op.tlpParams.pw();
      PrintWriter pwErr = writer;//op.tlpParams.pw(System.err);
      LexicalizedParserQuery pq = lp.parserQuery();
      if (pq.parse(op.tlpParams.defaultTestSentence())) {
        op.testOptions.treePrint(op.tlpParams).printTree(pq.getBestParse(), pwOut);
      } else {
      ...
      // We parse filenames given by the remaining arguments
      lp.parserQuery().parseFiles(args, argIndex, tokenized, tokenizerFactory, elementDelimiter, sentenceDelimiter, escaper, tagDelimiter, sentence_text, writer);
    }
  }
