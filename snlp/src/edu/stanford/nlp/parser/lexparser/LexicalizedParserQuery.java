...
public class LexicalizedParserQuery {
  ...
  /* 
   * Yuri added - Parse the sentence text in the String array args elements from
   */
  void parseFiles(String[] args, int argIndex, boolean tokenized, TokenizerFactory<? extends HasWord> tokenizerFactory, String elementDelimiter, String sentenceDelimiter, Function<List<HasWord>, List<HasWord>> escaper, String tagDelimiter) {
	  parseFiles(args, argIndex, tokenized, tokenizerFactory, elementDelimiter, sentenceDelimiter, escaper, tagDelimiter, null, null);
  }

  /** Parse the files with names given in the String array args elements from
   *  index argIndex on.
   */
  void parseFiles(String[] args, int argIndex, boolean tokenized, TokenizerFactory<? extends HasWord> tokenizerFactory, String elementDelimiter, String sentenceDelimiter, Function<List<HasWord>, List<HasWord>> escaper, String tagDelimiter, String sentence_text, PrintWriter writer) {
    final TreebankLanguagePack tlp = op.tlpParams.treebankLanguagePack();
    final PrintWriter pwOut = writer != null ? writer : op.tlpParams.pw();
    final PrintWriter pwErr = writer != null ? writer : op.tlpParams.pw(System.err);
      ...
      if (filename.equals("-")) {
        try {
          ...
          if (sentence_text != null) {
        	  //File file = new File("TempSentenceFile");
        	  //StringUtils.printToFile(file, sentence_text);
        	  ////File file = File.createTempFile("TempSentenceFile", null);
              ////file.deleteOnExit();
              //documentPreprocessor = new DocumentPreprocessor(file.getAbsolutePath(), docType);
        	  documentPreprocessor = new DocumentPreprocessor(new StringReader(sentence_text), docType);
          } else {
